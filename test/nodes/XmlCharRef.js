...
describe("XmlCharRef", () => {
    ...
    describe("#char", () => {
        ...
        it("should throw an error if the specified value is not a" +
            " string", () => {
            let node = new XmlCharRef("a");
            assert.throws(() => node.char = undefined);
            assert.throws(() => node.char = null);
            assert.throws(() => node.char = 0);
            assert.throws(() => node.char = new XmlCharRef(""));
        });

        it("should throw an error if the specified value contains characters" +
            " not allowed in XML", () => {
            let node = new XmlCharRef("a");
            assert.throws(() => node.char = String.fromCodePoint(0x0001));
        });
    ...
    describe("#hex", () => {
        ...
        it("should throw an error if the specified value is not a" +
            " boolean", () => {
            let node = new XmlCharRef("a");
            assert.throws(() => node.hex = undefined);
            assert.throws(() => node.hex = null);
            assert.throws(() => node.hex = 0);
            assert.throws(() => node.hex = "test");
            assert.throws(() => node.hex = new XmlCharRef(""));
        });
    });
    ...
    describe("#toString", () => {
        it("should return a string containing the XML string representation" +
            ...
            node = new XmlCharRef(String.fromCodePoint(0xdbff) +
            ...
            node = new XmlCharRef(String.fromCodePoint(0xdbff) +
                String.fromCodePoint(0xdc00), true);
            assert.strictEqual(node.toString(), "&#x10fc00;");
        });
    });
});
