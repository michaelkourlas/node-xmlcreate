...
import {XmlComment, XmlDtd, XmlDtdAttlist, XmlDtdElement,
    XmlDtdEntity, XmlDtdNotation, XmlDtdParamEntityRef, XmlNode,
    XmlProcInst} from "../../lib/main";
...
describe("XmlDtd", () => {
    ...
    describe("#name", () => {
        ...
        it("should throw an error if the specified value is not a" +
            " string", () => {
            let node = new XmlDtd("abc");
            assert.throws(() => node.name = undefined);
            assert.throws(() => node.name = null);
            assert.throws(() => node.name = 0);
            assert.throws(() => node.name = new XmlDtd("abc"));
        });
    ...
    describe("#pubId", () => {
        ...
        it("should throw an error if the specified value is not a" +
            " string or undefined", () => {
            let node = new XmlDtd("abc", "def");
            assert.throws(() => node.pubId = null);
            assert.throws(() => node.pubId = 0);
            assert.throws(() => node.pubId = new XmlDtd("abc"));
        });
    ...
    describe("#sysId", () => {
        ...
        it("should throw an error if the specified value is not a" +
            " string or undefined", () => {
            let node = new XmlDtd("abc");
            assert.throws(() => node.sysId = null);
            assert.throws(() => node.sysId = 0);
            assert.throws(() => node.sysId = new XmlDtd("abc"));
        });
        ...
        it("should throw an error if the specified value contains characters" +
            ...
            assert.throws(() => node.sysId = "abc" +
                String.fromCodePoint(0x0001) + "def");
        ...
        it("should throw an error if the specified value is undefined when" +
            " pubId is defined", () => {
            let node = new XmlDtd("abc", "def", "ghi");
            assert.throws(() => node.sysId = undefined);
        });
    });
    ...
    describe("#paramEntityRef", () => {
        it("should add an XmlDtdParamEntityRef node to this node's children" +
            ...
            let node = new XmlDtd("abc");
            assert.isTrue(node.paramEntityRef("test") instanceof
                XmlDtdParamEntityRef);
            assert.isTrue(node.paramEntityRef("test2", 0) instanceof
                XmlDtdParamEntityRef);
            assert.isTrue(node.paramEntityRef("test3", 1) instanceof
                XmlDtdParamEntityRef);
            assert.strictEqual(node.toString(), "<!DOCTYPE abc [\n" +
                "    %test2;\n    %test3;\n    %test;\n]>");
        });
    ...
    describe("#procInst", () => {
        it("should add an XmlProcInst node to this node's children" +
            ...
            assert.isTrue(node.procInst("test2", "b", 0) instanceof
                XmlProcInst);
            assert.isTrue(node.procInst("test3", "c", 1) instanceof
                XmlProcInst);
            assert.isTrue(node.procInst("test4") instanceof XmlProcInst);
            assert.isTrue(node.procInst("test5", undefined, 0) instanceof
                XmlProcInst);
            assert.strictEqual(node.toString(), "<!DOCTYPE abc [\n" +
                "    <?test5?>\n    <?test2 b?>\n    <?test3 c?>\n" +
                "    <?test a?>\n    <?test4?>\n]>");
