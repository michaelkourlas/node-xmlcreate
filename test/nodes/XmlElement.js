...
import {XmlAttribute, XmlCdata, XmlCharRef, XmlComment, XmlElement,
    XmlEntityRef, XmlNode, XmlProcInst, XmlText} from "../../lib/main";
...
describe("XmlElement", () => {
    ...
    describe("#name", () => {
        ...
        it("should throw an error if the specified value is not a" +
            ...
            assert.throws(() => node.name = new XmlElement(""));
        });
    ...
    describe("#attribute", () => {
        it("should add an XmlAttribute node to this node's children at the" +
            ...
            assert.isTrue(node.attribute("test", "test1")
                instanceof XmlAttribute);
            assert.isTrue(node.attribute("test2", "test3", 0)
                instanceof XmlAttribute);
            assert.isTrue(node.attribute("test4", "test5", 1)
                instanceof XmlAttribute);
