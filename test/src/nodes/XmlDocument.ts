...
describe("XmlDocument", () => {
    ...
    describe("#dtd", () => {
        it("before root element", () => {
            ...
            assert.isTrue(node.decl({}) instanceof XmlDecl);
            assert.isTrue(
                node.dtd({name: "abc", sysId: "def", pubId: "ghi"}) instanceof
                    XmlDtd
            );
            assert.isTrue(node.element({name: "root"}) instanceof XmlElement);
            assert.strictEqual(
    ...
    it("#procInst", () => {
        ...
        assert.isTrue(
            node.procInst({target: "def", content: "ghi"}) instanceof
                XmlProcInst
        );
        assert.strictEqual(node.toString(), "<root/>\n<?abc?>\n<?def ghi?>");
    });
    ...
    describe("#toString", () => {
        function getXmlDocument(): XmlDocument {
            ...
            // @formatter:off
            node.dtd({name: "stop"})
                .attlist({charData: "purpose type CDATA 'retire'"})
                .up()
                .comment({charData: "rely bite-sized magic"})
                .up()
                .element({charData: "noiseless EMPTY"})
                .up()
                .entity({charData: "protective 'multiply'"})
                .up()
                .notation({charData: "murky PUBLIC 'tested gratis'"})
                .up()
                .paramEntityRef({name: "giddy"})
                .up()
                .procInst({target: "pencil", content: "four design"})
                .up()
                .up();
            // @formatter:on
            ...
            // @formatter:off
            node.element({name: "root"})
                .attribute({name: "att"})
                .text({charData: "text"})
                .up()
                .up()
                .element({name: "ele"})
                .comment({charData: "cmt"})
                .up()
                .element({name: "subele"})
                ...
                .up()
                .up()
                .procInst({target: "proc", content: "cont"})
                .up()
                .entityRef({name: "ent"})
                .up()
                .up()
                .element({name: "ele2"})
                .charData({charData: "cha"})
                .up()
                .charRef({char: "c"})
                .up()
                .cdata({charData: "cda"});
        ...
        it(
            "no root element; default quotes; default pretty printing;" +
        ...
        it(
            "root element and other children; default quotes; default pretty" +
        ...
        it(
            "root element and other children; single quotes; default pretty" +
                " printing; default indentation; default newline",
            () => {
                const node = getXmlDocument();
                assert.strictEqual(
        ...
        it(
            "root element and other children; double quotes; default pretty" +
                " printing; default indentation; default newline",
            () => {
                const node = getXmlDocument();
                assert.strictEqual(
        ...
        it(
            "root element and other children; default quotes; pretty" +
